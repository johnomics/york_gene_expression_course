...
Before we start, reload your Sleuth analysis from yesterday so we can look at the results as we go through:
...
We'll come back to the `read_bootstrap_tpm` argument later (in the "Types of variance" and "TPMs" sections). Here, we'll start to explore what it means to fit a model.
...
We have made the `sleuth_object_human.rds` file available so you don't have to do your own analysis. Download this file by clicking the button below (this will take some time and it may look as if nothing is happening - please be patient), then move the file to your project folder:

```{r, echo=FALSE}
fixedRow(
  column(12, 
    downloadButton("downloadHuman", "Download human Sleuth data")
  )
)
fixedRow()
```

```{r human download, context="server"}
output$downloadHuman<-downloadHandler(
  filename = function() {"sleuth_object_human.rds"},
  content = function(file) {
    human_data <- sleuth_load("sleuth_object_human.rds")
    sleuth_save(human_data, file)
  }
)
```
